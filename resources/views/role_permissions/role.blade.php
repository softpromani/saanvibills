    ...
    <div class="container">
        ...
        <div class="page-meta">
            <nav class="breadcrumb-style-one" aria-label="breadcrumb">
                <ol class="breadcrumb">
                    ...
                    <li class="breadcrumb-item active" aria-current="page">{{__('sidebar.role')}}</li>
                </ol>
            </nav>
        </div>
        <!-- /BREADCRUMB -->
        <div class="page-meta card p-3 ">
            <form action="{{ url('admin/create_role') }}" method="POST">
                @csrf
                <div class="input-group">
                    <input type="text" class="form-control" name="role" placeholder="Role Name"
                        aria-label="Recipient's username with two button addons">
                    <button class="btn btn-outline-primary" type="submit">{{__('sidebar.create role')}}</button>
                </div>
            </form>
        </div>
        ...
        <div id="customTables" class="col-lg-12 mt-4">
            <div class="seperator-header">
                <h4 class="">{{__('sidebar.role list')}}</h4>
            </div>
        </div>
                ...
                <div class="widget-content widget-content-area">
                    ...
                    <div class="table-responsive">
                        <table class="table">
                            <thead>
                                <tr>
                                    <th scope="col">{{__('form.sr_no')}}</th>
                                    <th scope="col">{{__('form.role name')}}</th>
                                    <th class="text-center" scope="col">{{__('form.status')}}</th>
                                    <th class="text-center" scope="col">{{__('form.action')}}</th>
                                </tr>
